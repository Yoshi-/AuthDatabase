		...
		if(isset($_POST['securitytoken']) AND isset($_SESSION['token']) AND $_SESSION['token'] == $_POST['securitytoken']) {
			...
			if(isset($_POST['Auths'])) {
				...
				foreach($matches[0] as $value) {
					...
					$insert .= '(NULL , "'.mysql_real_escape_string(htmlentities($value)).'", "0", "'.mysql_real_escape_string(htmlentities($key[0])).'"),';
				}
				$insert = trim($insert, ',');
				$query = "INSERT IGNORE INTO `auths` (`authID` ,`auth` ,`working` ,`key`) ". $insert;
